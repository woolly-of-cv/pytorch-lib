...
def train_native(use_l1=False, lambda_l1=5e-4):
    ...
    def internal(model, train_loader, optimizer, criteria, dropout, device, scheduler=None):
        ...
        for data, target, weightage, ricap in train_loader:
            data, target, weightage, ricap = data.to(device), [t.to(device) for t in target], [
                w.to(device) for w in weightage], ricap.to(device)
            ...
            batch_correct = pred.eq(
                target[0][false_indices].view_as(pred)).sum().item()

            # Metrics for Ricap based
            pred = output[true_indices].argmax(dim=1, keepdim=True)
            for k in range(4):
                loss = loss + (weightage[k][true_indices] * criteria(
                    output[true_indices], target[k][true_indices], reduction='none')).mean()

                batch_correct += (weightage[k][true_indices] * pred.eq(
                    target[k][true_indices].view_as(pred)).float().t()).sum().item()
...
class LabelSmoothingLoss(nn.Module):
    def __init__(self, classes=10, smoothing=0.0, dim=-1, weight=None):
...
def get_label_smoothing_criteria(device, smoothing=0.2):
    return LabelSmoothCrossEntropyLoss(smoothing=smoothing)
