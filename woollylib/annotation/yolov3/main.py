...
MAIN_COLORS = ['red', 'blue', 'black', 'yellow', 'green', 'darkolivegreen', 'darkseagreen', 'darkorange', 'darkslategrey', 'darkturquoise', 'darkgreen', 'darkviolet', 'darkgray', 'darkmagenta', 'darkblue', 'darkkhaki', 'darkcyan',
               'darkred',  'darksalmon', 'darkslategray', 'darkgoldenrod', 'darkgrey', 'darkslateblue', 'darkorchid', 'skyblue', 'orange', 'pink', 'violet', 'brown', 'gold', 'Olive', 'Maroon', 'cyan', 'olivedrab', 'lightcyan', 'silver']
...
class LabelTool():
    def __init__(self, master):
        ...
        self.entry.insert(0, os.path.join(os.getcwd()))
        self.entry.focus_set()
        self.entry.bind('<Return>', self.loadEntry)
        self.entry.bind('<Control-KeyRelease-a>',
                        self.select_text_or_select_and_copy_text)
        self.entry.grid(row=0, column=1, sticky=W+E)
        self.explBtn = Button(self.frame,
                              text="b",
                              command=self.dispPath)
        self.explBtn.grid(row=0, column=2, sticky=W+S+E)
        self.ldBtn = Button(self.frame, text="Load",
                            width=23, command=self.loadDir)
        ...
        self.popupMenu = OptionMenu(
            self.frame, self.tkvar, *classes, command=self.change_dropdown)
        self.popupMenu.grid(row=1, column=3, sticky=E+S)
        self.indd = classes.index(classes[0])
        self.widge = self.popupMenu
        self.widge.config(bg=COLORS[self.indd], fg='black',
                          activebackground=COLORS[self.indd],    activeforeground='black')
        for ind, j in enumerate(classes):
            self.popupMenu['menu'].entryconfig(
                ind, activebackground=COLORS[ind], background=COLORS[ind])
    ...
    def loadDir(self, dbg=False):
        ...
        if len(self.imageList) == 0:
            ...
            tkMessageBox.showerror(
                "Error!", message="No .jpeg/.jpg images found in the specified dir!")
            return
            ...
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    ...
                    self.listbox.itemconfig(
                        len(self.bboxIdList) - 1, fg=COLORS[int(yolo_data[0])])
    ...
    def mouseClick(self, event):
        if self.STATE['click'] == 0:
            ...
            self.bboxId = None
            self.listbox.insert(END, '(%d, %d) -> (%d, %d) -> (%s)' %
                                (x1, y1, x2, y2, classes[self.cur_cls_id]))
            self.listbox.itemconfig(
                len(self.bboxIdList) - 1, fg=COLORS[self.cur_cls_id])
    ...
    def prevImage(self, event=None):
        ...
        if self.cur > 1:
            ...
            tkMessageBox.showerror(
                "Information!", message="This is first image")
    ...
    def nextImage(self, event=None):
        ...
        if self.cur < self.total:
            ...
            tkMessageBox.showerror(
                "Information!", message="All images annotated")
